...
@interface HDArriveViewController ()

@property(nonatomic,strong)BMKUserLocation *currentLocation;
@end

@implementation HDArriveViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        _currentLocation = [[BMKUserLocation alloc]init];
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    [toolButton setImage:[UIImage imageNamed:@"backb.png"] forState:UIControlStateNormal];
    [toolButton addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];

    
    mapView = [[BMKMapView alloc]initWithFrame:CGRectMake(0, 0, 320, 568)];
...
{
	if (userLocation != self.currentLocation) {
		NSLog(@"%f %f %@", userLocation.location.coordinate.latitude, userLocation.location.coordinate.longitude,userLocation.title);
...
-(void)searchRoute
{
	...
	//start.name = @"";
    start.pt = self.currentLocation.coordinate;
    
	BMKPlanNode* end = [[BMKPlanNode alloc]init] ;
    
    
    CLLocationCoordinate2D point = CLLocationCoordinate2DMake(36.657704,117.098035);
    end.pt = point;
    
    BMKSearch *search = [[BMKSearch alloc]init];
    search.delegate = self;
    
    BOOL flag = [search drivingSearch:nil startNode:start endCity:@"济南" endNode:end];
	if (flag) {
...
- (void)onGetDrivingRouteResult:(BMKPlanResult*)result errorCode:(int)error
{
    if (result != nil) {
        ...
        if (error == BMKErrorOk) {
            ...
            [mapView setCenterCoordinate:result.startNode.pt animated:YES];
        }
    }
}
